...
static int group_setup(void **state) {
  ...
  *start_body = mg_str(start_info_buf);
  ...
  char *update_info_buf =
      malloc(5 * REQUEST_FIELD_SHORT_SIZE + 2 * sizeof(time_t));
  snprintf(update_info_buf, 5 * REQUEST_FIELD_SHORT_SIZE + 2 * sizeof(time_t),
           ...
           "end_time=%ld",
           s->default_update_info->info->client,
           s->default_update_info->info->project,
           s->default_update_info->info->activity,
           s->default_update_info->info->description,
           s->default_update_info->start_time,
           s->default_update_info->end_time);
...
static int group_teardown(void **state) {
  ...
  if (s) {
    ...
    if (s->TEST_TIMER_RESULT) {
      // Only free the outer pointers since TEST_TIMER_RESULT strings lives on
      // the stack
