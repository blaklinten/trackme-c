...
static template *newtemplate(const char *filename, const char *tmplstr,
  ...
  if (tmplstr == 0) {
    if ((fp = fopen(filename, "r")) != 0 && fstat(fileno(fp), &stb) == 0 &&
        ...
        (stb.st_size == 0 ||
         (stb.st_size > 0 &&
          fread(buf, 1, stb.st_size, fp) == (unsigned)stb.st_size))) {
      fclose(fp);
      buf[stb.st_size] = 0;
    } else {
      if (errout != 0) {
...
TMPL_varlist *TMPL_add_loop(TMPL_varlist *varlist, const char *name,
                            TMPL_loop *loop) {
  TMPL_loop *lp;
...
TMPL_fmtlist *TMPL_add_fmt(TMPL_fmtlist *fmtlist, const char *name,
                           TMPL_fmtfunc fmtfunc) {
  TMPL_fmtlist *newfmt;
  if (name == 0 || fmtfunc == 0) {
    return fmtlist;
